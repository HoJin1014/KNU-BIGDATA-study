...
# 총 금액을 저장해야되기에 sum = 0으로 표현


#from openpyxl import load_workbook    # 엑셀 사용하지 못해서 from tkinter import messagebox와 from datetime import datetime 사이 (4번줄)


# wb = load_workbook("drink_order.xlsx")                                             # now_dt 시간 설명과 file name 사이 참조 (47~50줄)
    # ws = wb['Sheet1']
...
# 이름, 휴대폰 사용할 때 다시 사용!!!                  # label2, label3 위에 사용해서 frame3.pack() 과 label1 사이에서 가져옴   (160줄)
...
# row 참조   label2, 142줄      row 10 , column 6
# row 참조   label3, 143줄      row 10 , column 8


# btn_16.grid(row=3, column=5, padx=10, pady=10) (149줄, 다른 줄에 대체)
# btn_17 = tk.Button(frame1, text="머먹조 CAFE KIOSK", width=10, height=2) #####              # 91줄 버튼 넣으려다가 뺌


#titlelabel1.grid(row=0, column=0, padx=10, pady=10) #####121줄 수정하기 전


# frame2 = tk.Frame(window)                   # 윗 줄에 대체    entry2.grid와 label1 사이 (156줄~ 160줄)
# frame2.pack()
#
# frame3 = tk.Frame(window)
# frame3.pack()