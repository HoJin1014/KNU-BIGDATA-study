...
def send():
    ...
    ws = wb['Sheet1']
    ws.append([now_dt, name, hp, order2['에스프레소'], order2['아메리카노'], order2['카페라떼'], order2['카페모카'], order2['카푸치노'], order2['초코케이크'], order2['치즈케이크'], order2['타르트'], order2['베이글'], order2['마카롱'], sum])
...
orderlabel.grid(row=2, column=0, padx=10, pady=10)
entry1 = tk.Entry(frame1).grid(row=2, column=1)
entry2 = tk.Entry(frame1).grid(row=2, column=3)

btn_11.grid(row=2, column=5, padx=10, pady=10)
